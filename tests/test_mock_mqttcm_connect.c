...
{ 	
 	...
 	will_return (mosquitto_lib_init, 0);
  	...
  	const char* filepath = "/tmp/.mqttconfig";
    	char* content1 = "CA_FILE_PATH=/xxx/yyy/zzzzzz/plume_ca.pem\n";
    	char* content2 = "CERT_FILE_PATH=/aaa/bbbbb/client.pem\n";
    	char* content3 = "KEY_FILE_PATH=/bbb/cccc/mqttcm/jlyauhtie.mvf\n";
	
	/*FILE* file1 = fopen(filepath, "w"); 
    	if(NULL != file1) {
    		fputs(content1, file1);
    		fputs(content2, file1);
    		fputs(content3, file1);
    		fclose(file1);
    	}
    	else {
	...
	int file_descriptor1 = open(filepath, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	...
	/*FILE* file2 = fopen(devicePropsFile, "w"); 
        if(NULL != file2) {
        	fputs(content, file2);
        	fclose(file2);
        }
        ...
        if (chmod(devicePropsFile, 0644) != 0) {
        	printf("---------------Error setting file permissions---------------\n");
    	}*/
    	
    	int file_descriptor2 = open(devicePropsFile, O_WRONLY | O_CREAT | O_TRUNC, 0644);
